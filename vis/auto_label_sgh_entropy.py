#!/usr/bin/env python3
"""
深圳-东莞-惠州市内部流动自动格网标注脚本（基于Shannon Entropy版本）

工作流程：
1. 加载深莞惠格网列表
2. 从深莞惠CSV数据中提取格网的24小时流量和OD分布
...
OUTPUT_LABELS = LABELS_DIR / "labels_sgh_entropy.csv"
OUTPUT_EDGE_CASES = LABELS_DIR / "edge_cases_sgh_entropy.csv"
...
class SGHAutoLabelerEntropy:
    """基于Shannon Entropy的深莞惠自动标注器"""
    ...
    def load_sgh_grids(self):
        ...
        df = pd.read_csv(SGH_GRID_METADATA)

        # 筛选深莞惠格网
        sgh_df = df[df['city_name'].isin(['深圳市', '东莞市', '惠州市'])]
        self.sgh_grids = set(sgh_df['grid_id'].values)

        print(f"    ✓ 深莞惠格网数量: {len(self.sgh_grids):,}")
        print(f"      - 深圳市: {len(sgh_df[sgh_df['city_name'] == '深圳市']):,}")
        print(f"      - 东莞市: {len(sgh_df[sgh_df['city_name'] == '东莞市']):,}")
        print(f"      - 惠州市: {len(sgh_df[sgh_df['city_name'] == '惠州市']):,}")
    ...
    def load_sgh_flow_data(self, year: int) -> pd.DataFrame:
        """加载深莞惠流动数据（CSV已经是深莞惠内部流动数据）"""
        if year == 2021:
            filepath = SGH_DATA_2021
        else:
            filepath = SGH_DATA_2024

        print(f"[*] 加载 {year} 年深莞惠数据: {filepath}")

        # 直接读取CSV，无需过滤（数据已经是深莞惠内部流动）
        df = pd.read_csv(filepath)
        print(f"    ✓ {year}年深莞惠内部流动记录数: {len(df):,}")
        return df

    def compute_all_hourly_data(self):
        """计算所有深莞惠格网的24小时流量数据和OD分布"""
        print("\n[*] 计算格网24小时流量数据和OD分布...")

        # 加载两年数据
        df_2021 = self.load_sgh_flow_data(2021)
        df_2024 = self.load_sgh_flow_data(2024)

        # 计算每个格网的数据
        print(f"[*] 处理2021年数据...")
        for grid_id in tqdm(self.sgh_grids, desc="2021年"):
        ...
        print(f"[*] 处理2024年数据...")
        for grid_id in tqdm(self.sgh_grids, desc="2024年"):
    ...
    def run_batch(self, max_count: int = None):
        """批量标注所有深莞惠格网"""
        print("=" * 80)
        print("深圳-东莞-惠州市内部流动自动标注（基于Shannon Entropy）")
        print("=" * 80)
