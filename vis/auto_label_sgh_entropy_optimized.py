#!/usr/bin/env python3
"""
深圳-东莞-惠州市内部流动自动格网标注脚本（基于Shannon Entropy版本 - 优化版）
...
SGH_DATA_2021 = DATA_DIR / "2021_sgh_week.csv"
SGH_DATA_2024 = DATA_DIR / "2024_sgh_week.csv"
SGH_GRID_METADATA = DATA_DIR / "grid_metadata" / "PRD_grid_metadata.csv"
# 确保缓存目录存在
CACHE_DIR.mkdir(parents=True, exist_ok=True)

# 熵变阈值（用于判断聚集/扩散）
ENTROPY_CHANGE_THRESHOLD = 0.03 # 3%变化视为显著

# 输出文件路径（动态包含阈值）
OUTPUT_LABELS = LABELS_DIR / f"labels_sgh_entropy_{ENTROPY_CHANGE_THRESHOLD}.csv"
OUTPUT_EDGE_CASES = LABELS_DIR / f"edge_cases_sgh_entropy_{ENTROPY_CHANGE_THRESHOLD}.csv"
...
class SGHAutoLabelerEntropyOptimized:
    """基于Shannon Entropy的深莞惠自动标注器（优化版）"""

    def __init__(self):
        self.sgh_grids: Set[int] = set()
        self.grid_metadata: pd.DataFrame = None

    def load_sgh_grids(self):
        """加载深莞惠格网列表"""
        print(f"[*] 加载深莞惠格网列表: {SGH_GRID_METADATA}")
        self.grid_metadata = pd.read_csv(SGH_GRID_METADATA)

        # 筛选深莞惠格网
        sgh_df = self.grid_metadata[self.grid_metadata['city_name'].isin(['深圳市', '东莞市', '惠州市'])]
        self.sgh_grids = set(sgh_df['grid_id'].values)

        print(f"    ✓ 深莞惠格网数量: {len(self.sgh_grids):,}")
        print(f"    ✓ 包含城市: {', '.join(sgh_df['city_name'].unique())}")
        return self.sgh_grids

    def show_cache_info(self):
        """显示缓存信息"""
        print("[*] 缓存信息:")
        for year in [2021, 2024]:
            cache_file = CACHE_DIR / f"processed_sgh_{year}.pkl"
    ...
    def preprocess_data(self, filepath: Path, year: int, use_cache: bool = True) -> Tuple[pd.DataFrame, Dict[int, float], Dict[int, Dict[int, float]], Dict[int, np.ndarray]]:
        ...
        od_distributions = {}
        for grid_id in tqdm(self.sgh_grids, desc=f"   {year}年OD", leave=False):
        ...
        for grid_id in tqdm(self.sgh_grids, desc=f"   {year}年hourly", leave=False):
    ...
    def run_batch(self, num_labels: int = None, use_cache: bool = True):
        ...
        """
        print("=" * 80)
        print("深圳-东莞-惠州市内部流动自动标注（基于Shannon Entropy - 优化版）")
        print("=" * 80)
        ...
        # 预处理数据
        _, daily_totals_2021, od_dist_2021, _ = self.preprocess_data(SGH_DATA_2021, 2021, use_cache)
        _, daily_totals_2024, od_dist_2024, _ = self.preprocess_data(SGH_DATA_2024, 2024, use_cache)

        # 确定要处理的格网列表
        grids_to_process = list(self.sgh_grids)
            ...
            try:
                ...
                if is_edge:
                    ...
                    edge_cases_data.append({
                        ...
                        'entropy_2021': entropy_2021,
                        'entropy_2024': entropy_2024,
                        ...
                        'is_edge_trend': is_edge_trend,
                        'is_edge_spatial': is_edge_spatial
                    })
        ...
        # 保存边缘案例
        if edge_cases_data:
            print(f"[*] 保存边缘案例到: {OUTPUT_EDGE_CASES}")
        ...
        for label_id in range(1, 10):
            ...
            if count > 0:
                print(f"  标签 {label_id} ({LABEL_MAP[label_id]}): {count:,} 个格网 ({count/stats['success']*100:.1f}%)")
...
def main():
    ...
    num_labels = None  # 默认处理所有格网
    if len(sys.argv) > 1:
        num_labels = int(sys.argv[1])

    labeler = SGHAutoLabelerEntropyOptimized()
    labeler.run_batch(num_labels=num_labels)
